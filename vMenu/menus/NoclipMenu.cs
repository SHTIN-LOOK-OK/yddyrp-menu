...
namespace vMenuClient
{
    public class NoclipMenu : BaseScript
    {
        ...
        private async Task OnTick()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        noclipMenu.InstructionalButtons[Control.Sprint] = $"Изменить скорость ({speeds[currentSpeed]})";
        ...
        private void Setup()
        {
            noclipMenu = new Menu("No Clip", "Управление") { IgnoreDontOpenMenus = true };

            MenuItem speed = new MenuItem("Текущая скорость", "")
            {
            ...
            /*noclipMenu.OnMenuOpen += (m) =>
            {
                if (MainMenu.NoClipEnabled)
                    HelpMessage.Custom("No Clip активен. Пожалуйста, пользуйтесь этой функцией разумно.");
            };*/

            noclipMenu.AddMenuItem(speed);

            noclipMenu.InstructionalButtons.Clear();
            noclipMenu.InstructionalButtons.Add(Control.Sprint, $"Изменить скорость ({speeds[currentSpeed]})");
            noclipMenu.InstructionalButtons.Add(Control.MoveUpDown, "Вперед/Назад");
            noclipMenu.InstructionalButtons.Add(Control.MoveLeftRight, "Влево/Вправо");
            noclipMenu.InstructionalButtons.Add(Control.MultiplayerInfo, "Вниз");
            noclipMenu.InstructionalButtons.Add(Control.Cover, "Вверх");
            noclipMenu.InstructionalButtons.Add((Control)MainMenu.NoClipKey, "Отключить No Clip");
