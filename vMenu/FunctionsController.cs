                ...
                if (MainMenu.MiscSettingsMenu.JoinQuitNotifications && IsAllowed(Permission.MSJoinQuitNotifs))
                {
                    ...
                    if (pl.Count > playerList.Count)
                    {
                        foreach (KeyValuePair<int, string> player in pl)
                        {
                            ...
                            {
                                Notify.Custom($"~g~<C>{GetSafePlayerName(player.Value)}</C>~s~ вошел.");
                                await Delay(0);
                            }
                        }
                    }
                    ...
                    else if (pl.Count < playerList.Count)
                    {
                        foreach (KeyValuePair<int, string> player in playerList)
                        {
                            ...
                            {
                                Notify.Custom($"~r~<C>{GetSafePlayerName(player.Value)}</C>~s~ вышел.");
                                await Delay(0);
                            }
                        }
                    }
                    ...
                    foreach (Player p in pl)
                    {
                        ...
                        if (p.IsDead)
                        {
                            ...
                            if (killer != null)
                            {
                                    ...
                                    if (killer.Exists())
                                    {
                                        if (killer.Model.IsPed)
                                        {
                                            ...
                                            foreach (Player playerKiller in pl)
                                            {
                                                ...
                                                {
                                                    Notify.Custom($"~o~<C>{GetSafePlayerName(p.Name)}</C> ~s~был убит ~y~<C>{GetSafePlayerName(playerKiller.Name)}</C>~s~.");
                                                    found = true;
                                                    break;
                                                }
                                            }
                                            if (!found)
                                            {
                                                Notify.Custom($"~o~<C>{GetSafePlayerName(p.Name)}</C> ~s~был убит.");
                                            }
                                        }
                                ...
                                {
                                    Notify.Custom($"~o~<C>{GetSafePlayerName(p.Name)}</C> ~s~самоубилс€.");
                                }
                            }
                            else
                            {
                                Notify.Custom($"~o~<C>{GetSafePlayerName(p.Name)}</C> ~s~умер.");
                            }
                    ...
                    if (MainMenu.VoiceChatSettingsMenu.ShowCurrentSpeaker && IsAllowed(Permission.VCShowSpeaker))
                    {
                        ...
                        foreach (Player p in pl)
                        {
                            if (NetworkIsPlayerTalking(p.Handle))
                            {
                                if (!currentlyTalking)
                                {
                                    DrawTextOnScreen("~s~—ейчас говорит:", 0.5f, 0.00f, 0.5f, Alignment.Center, 6);
